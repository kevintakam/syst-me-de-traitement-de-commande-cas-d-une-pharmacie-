...
if(isset($_POST['search'])){
    ...
    if(is_numeric($_POST['search'])){
        $d= $z->rech_pat($_POST['search']);
    }else{
            $d= $z->rech_pat(NULL,$_POST['search']);
        }
    if($d !=FALSE){
           require 'header.php';
            ?>
                            ...
                            <div class="card-body">
                                 <div class="row"><div class="col-sm-12"><table id="bootstrap-data-table-export" class="table table-striped table-bordered dataTable no-footer " role="grid" aria-describedby="bootstrap-data-table-export_info"> 
                                    <thead>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="bootstrap-data-table-export" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending" style="width: 140px;">Action</th></tr>
                                    </thead>
                                    <tbody>
                                              <?php 
                                              foreach ($d as $r):
                                          ...
                                          <div class="col-md-6"><button name ='id' class= "btn btn-success"><a href="consulter_commande.php/?id=<?php echo $r->get_id(); ?>&reference=<?php echo $r->getCommandes()[$i]->getRef(); ?>&commande=<?php echo $r->getCommandes()[$i]->getId(); ?>."style='color:white;' ">consulter commande</a></button></div>
                                         </td>
                                         <?php }else{?>
...
<?php die();?>
     <?php    
        ...
        echo '<h1>AUCUN RESULTAT TROUVE</h1>';
    }
